		...
		<div className="flex min-h-screen flex-col items-center justify-center gap-4 px-6">
			<div className="grid items-center gap-16 md:flex">
				<ErrorContent
					...
					description={
						notFound
							? t('errors.playerNotFound.description')
							: t('errors.routeError.description')
					}
					onButtonClick={notFound ? undefined : retry}
				/>
				<Image
					alt={notFound ? 'player not found' : 'error'}
					className={`rounded-lg p-3 dark:bg-transparent ${
						notFound ? 'bg-neutral-200' : 'bg-neutral-400'
					}`}
					height={400}
					src={
						notFound
