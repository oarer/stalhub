...
export default function Page() {
	...
	useEffect(() => {
		...
		if (desktopState && codeChallenge) {
			...
			setDesktopStatus('exchanging')
			issueDesktopLoginUrl()
				.then((url) => {
					window.location.replace(url)
					setDesktopStatus('closeable')
				})
				.catch(() => {
	...
	return (
		<section className="relative flex min-h-dvh w-full items-center justify-center px-4 py-8">
			{desktopStatus && (
				<div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-2 text-center">
					{desktopStatus === 'exchanging' ? (
						<>
							<Icon
