...
export default function UserComments({ userId }: UserCommentsProps) {
	...
	const createMutation = useMutation({
		mutationFn: () =>
			userCommentService.create(userId, {
	...
	return (
		<div className="flex flex-col gap-4">
			<h3 className="font-semibold text-lg">
				{t('users.comments.title', { count: data.total_count })}
			</h3>
				...
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-2">
						<Image
							...
							height={42}
							src={`${process.env.NEXT_PUBLIC_CDN_URL}/api/v1/users/avatar/${comment.author.id}`}
							unoptimized
							width={42}
						/>
						...
						{canDelete && (
							<Modal.Root>
								...
								<Modal.Content fullScreen={false}>
									<Modal.Header>
										<Modal.Title>
											{t('users.comments.deleteTitle')}
										</Modal.Title>
									</Modal.Header>
									<Modal.Footer>
										<Modal.Close>
											{t('users.comments.cancel')}
										</Modal.Close>
										<Modal.Action
											closeOnClick
